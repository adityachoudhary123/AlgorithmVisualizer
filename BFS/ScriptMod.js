...
function setDestTiles() {
    if(setDestTilesButton) {
        setDestTilesButton=false;
        document.getElementById("SetDest").style.background = "#afb0a5";
    }
    else {
        setDestTilesButton = true;
        document.getElementById("SetDest").style.background = "green";
    }
}
...
function clickTile() {
    ...
    if(setDestTilesButton) {
        ...
        destC = parseInt(coords[1]);
        destVert.push([destR,destC]);
        destMap.push(destR.toString() + "-" + destC.toString());
    }
    else {
...
function submit() {
    ...
    //console.log(destVert);
    for(let i = 0; i < destVert.length; i++) {
        let cord = destVert[i];
        console.log(cord[0].toString() + "-" + cord[1].toString());
        shortestPath(cord[0],cord[1]);
    }
    console.log(shortDist);
    //shortestPath();
    // console.log(shortDist);
    for(let i = 0; i < shortDist.length; i++) {
        displayPath(shortDist[i]);
    }

}

function shortestPath(dx, dy) {
    var sx = sourceR; 
    var sy = sourceC;
    // var dx = destR;
    // var dy = destC;	
    ...
    while ((p = queue.shift()) != null) {
        ...
        if(!destMap.includes(p.x.toString() + "-" + p.y.toString()) && !(p.x == sx && p.y == sy)) {
            setTimeout(() => {
                document.getElementById(ids).style.backgroundColor = "#d9bea5";
            },(index++)*10);
        }
